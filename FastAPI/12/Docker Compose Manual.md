# Посібник з використання Docker Compose: Основи та практичні приклади

- [Необхідні початкові умови](#необхідні-початкові-умови)
- [Що таке docker-compose.yml?](#що-таке-docker-composeyml)
- [Синтаксис команди docker compose](#синтаксис-команди-docker-compose)
- [Перевірка версії Docker та Docker Compose](#перевірка-версіі-docker-та-docker-compose)
- [Структура директорій із файлами](#структура-директорій-із-файлами)
- [Перевірка запущених та не запущених сервісів](#перевірка-запущених-та-не-запущених-сервісів)
- [Контент файлу `.env`](#контент-файлу-env)
- [Взаємодія контейнерів між собою в мережі](#взаємодія-контейнерів-між-собою-в-мережі)
- [Томи (volumes) контейнера](#томи-volumes-контейнера)
- [Зупинення контейнерів без їх видалення](#зупинення-контейнерів-без-їх-видалення)
- [Зупинення контейнерів та видалення їхніх пов'язаних даних](#зупинення-контейнерів-та-видалення-їхніх-повязаних-даних)
...
## Синтаксис команди docker compose
...
## Перевірка версії Docker та Docker Compose
...
### Структура директорій із файлами
```bash
.
├── backend
│   ├── book_api.py
...
## Перевірка запущених та не запущених сервісів
...
## Взаємодія контейнерів між собою в мережі
```yml
# docker-compose.yml
...
![docker-compose](./backend/image.png "Docker Compose Interaction")

Це зображення показує, що існує мережа з назвою `docker-network` та є 3 сервіси - `db`, `backend`, та `frontend`. `frontend` сервіс залежить від `backend` сервісу, а `backend` сервіс в свою чергу залежить від `db` сервісу. 

## Томи (volumes) контейнера
`db` сервіс має том (як розділ жорсткого диску на комп'ютері) `/var/lib/mysql` в середині контейнера. І на локальній машині цей том пов'язаний з томом `mysql_data`. В свою чергу `mysql_data` том розташований десь в системі на вашій локальній машині де у вас запущений docker compose. В цьому томі зберігаються необхідні вам або вашому контейнеру дані контейнера навіть після повного видалення контейнера. Якщо б його не було, то після видалення контейнера, всі дані також були б видалені.

## Зупинення контейнерів без їх видалення
```bash
docker-compose stop
```

## Зупинення контейнерів та видалення їхніх пов'язаних даних
Зупинити контейнери та видалити контейнери, мережі, томи та образи, створені через `docker-compose up ` можна наступною командою:
